...
Наземная станция поддерживает настраиваемые таймауты для обработки соединений:

| Параметр | Значение по умолчанию | Описание |
|----------|----------------------|----------|
| `connection_timeout` | 5.0s | Таймаут подключения к спутнику |
| `socket_timeout` | 2.0s | Таймаут сокетов для операций чтения/записи |
| `command_timeout` | 10.0s | Максимальное время ожидания ответа на команду |
| `max_idle_time` | 30s | Время бездействия до разрыва соединения |
| `max_retries` | 3 | Количество попыток повторной отправки команды |

### Настройка Таймаутов

Отредактируйте `config/ground_config.json`:
...
- **Таймаут соединения**: Если спутник не отвечает в течение `connection_timeout`, соединение считается неудачным
- **Таймаут бездействия**: Если нет активности в течение `max_idle_time`, соединение автоматически разрывается
- **Повторные попытки**: При неудачной отправке команды система делает до `max_retries` попыток
