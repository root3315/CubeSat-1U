...
class CommunicationHandler:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.stm32_serial: Optional[serial.Serial] = None  # type: ignore
        self.radio_serial: Optional[serial.Serial] = None  # type: ignore
    ...
    def init_serial_ports(self) -> None:
        ...
        try:
            # STM32 UART
            self.stm32_serial = serial.Serial(  # type: ignore
        ...
        try:
            # Radio UART
            self.radio_serial = serial.Serial(  # type: ignore
    ...
    def reader_loop(self) -> None:
        ...
        while self.running:
            # Read from STM32
            if self.stm32_serial and self.stm32_serial.in_waiting:  # type: ignore
                data: bytes = self.stm32_serial.read(self.stm32_serial.in_waiting)  # type: ignore
                self.process_stm32_data(data)

            # Read from radio
            if self.radio_serial and self.radio_serial.in_waiting:  # type: ignore
                data = self.radio_serial.read(self.radio_serial.in_waiting)  # type: ignore
    ...
    def send_to_stm32(self, data: Union[Dict[str, Any], bytes]) -> bool:
        ...
        try:
            if isinstance(data, dict):
                ...
                self.stm32_serial.write(cmd_data)  # type: ignore
            else:
                self.stm32_serial.write(data)  # type: ignore
    ...
    def send_to_radio(self, data: Union[Dict[str, Any], bytes]) -> bool:
        ...
        try:
            if isinstance(data, dict):
                ...
                self.radio_serial.write(json_str.encode())  # type: ignore
            else:
                self.radio_serial.write(data)  # type: ignore
        ...
        try:
            ...
            with open(filename, 'rb') as f:
                for chunk_num in range(num_chunks):
                    ...
                    # Send chunk
                    self.radio_serial.write(packet)  # type: ignore
    ...
    def cleanup(self) -> None:
        ...
        if self.stm32_serial:
            try:
                self.stm32_serial.cancel_read()  # Force any blocking reads to timeout  # type: ignore
            except:
                pass
            try:
                self.stm32_serial.close()  # type: ignore
            except Exception as e:
                self.logger.error(f"Error closing STM32 serial: {e}")

        if self.radio_serial:
            try:
                self.radio_serial.cancel_read()  # Force any blocking reads to timeout  # type: ignore
            except:
                pass
            try:
                self.radio_serial.close()  # type: ignore
            except Exception as e:
                self.logger.error(f"Error closing radio serial: {e}")

        if self.udp_socket:
            try:
                self.udp_socket.close()  # type: ignore
            except Exception as e:
