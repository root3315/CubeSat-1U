...
class TelemetryViewer:
    ...
    def setup_gui(self) -> None:
        ...
        self.paned: ttk.PanedWindow = ttk.PanedWindow(self.parent, orient=tk.HORIZONTAL)
        self.paned.pack(fill='both', expand=True)

        # Left panel - Current values
        left_frame: ttk.Frame = ttk.Frame(self.paned)
        ...
        # Right panel - Plots
        right_frame: ttk.Frame = ttk.Frame(self.paned)
        self.paned.add(right_frame, weight=3)

        # Create notebook for plot tabs
        plot_notebook: ttk.Notebook = ttk.Notebook(right_frame)
        plot_notebook.pack(fill='both', expand=True)

        # Temperature plot
        temp_frame: ttk.Frame = ttk.Frame(plot_notebook)
        plot_notebook.add(temp_frame, text="Temperature")
        self.setup_temp_plot(temp_frame)

        # Radiation plot
        rad_frame: ttk.Frame = ttk.Frame(plot_notebook)
        plot_notebook.add(rad_frame, text="Radiation")
        self.setup_rad_plot(rad_frame)

        # Magnetometer plot
        mag_frame: ttk.Frame = ttk.Frame(plot_notebook)
        plot_notebook.add(mag_frame, text="Magnetometer")
        self.setup_mag_plot(mag_frame)

        # Environment plot
        env_frame: ttk.Frame = ttk.Frame(plot_notebook)
        plot_notebook.add(env_frame, text="Environment")
        self.setup_env_plot(env_frame)

        # Battery plot
        bat_frame: ttk.Frame = ttk.Frame(plot_notebook)
        plot_notebook.add(bat_frame, text="Battery")
        self.setup_battery_plot(bat_frame)

    def setup_current_values(self, parent: tk.Frame) -> None:
        """Setup current values display"""
        # Create canvas with scrollbar
        canvas: tk.Canvas = tk.Canvas(parent)
        scrollbar: ttk.Scrollbar = ttk.Scrollbar(parent, orient="vertical", command=canvas.yview)
        scrollable_frame: ttk.Frame = ttk.Frame(canvas)
        ...
        # Title
        title: ttk.Label = ttk.Label(scrollable_frame, text="Current Telemetry",
                         font=('Arial', 14, 'bold'))
        title.grid(row=0, column=0, columnspan=2, pady=10)

        # Create labels for each telemetry value
        self.value_labels: Dict[str, Tuple[ttk.Label, str]] = {}
        telemetry_items: List[Tuple[str, str, Optional[str]]] = [
            ("Timestamp", "timestamp_str", None),
            ("System State", "system_state", None),
        ...
        for i, item in enumerate(telemetry_items):
            ...
            key: str = item[1]
            format_str: Optional[str] = item[2]
            ...
            # Value
            value_label: ttk.Label = ttk.Label(scrollable_frame, text="---",
                                   font=('Courier', 10))
            value_label.grid(row=i+1, column=1, sticky='w', padx=5, pady=2)

            self.value_labels[key] = (value_label, format_str if format_str else "{}")
        ...
        self.warning_label: ttk.Label = ttk.Label(scrollable_frame, text="None",
                                      foreground='green')
    ...
    def setup_temp_plot(self, parent: tk.Frame) -> None:
        """Setup temperature plot"""
        self.temp_fig: Figure = Figure(figsize=(6, 4), dpi=100)
        self.temp_ax: plt.Axes = self.temp_fig.add_subplot(111)  # type: ignore
        self.temp_line: plt.Line2D
        self.tmp_line: plt.Line2D
        ...
        self.temp_canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.temp_fig, master=parent)
        self.temp_canvas.get_tk_widget().pack(fill='both', expand=True)

    def setup_rad_plot(self, parent: tk.Frame) -> None:
        """Setup radiation plot"""
        self.rad_fig: Figure = Figure(figsize=(6, 4), dpi=100)
        self.rad_ax: plt.Axes = self.rad_fig.add_subplot(111)  # type: ignore
        self.rad_line: plt.Line2D
        ...
        self.rad_canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.rad_fig, master=parent)
        self.rad_canvas.get_tk_widget().pack(fill='both', expand=True)

    def setup_mag_plot(self, parent: tk.Frame) -> None:
        """Setup magnetometer plot"""
        self.mag_fig: Figure = Figure(figsize=(6, 4), dpi=100)
        self.mag_ax: plt.Axes = self.mag_fig.add_subplot(111)  # type: ignore

        self.mag_x_line: plt.Line2D
        self.mag_y_line: plt.Line2D
        self.mag_z_line: plt.Line2D
        ...
        self.mag_canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.mag_fig, master=parent)
        self.mag_canvas.get_tk_widget().pack(fill='both', expand=True)

    def setup_env_plot(self, parent: tk.Frame) -> None:
        """Setup environment plot (pressure, humidity)"""
        self.env_fig: Figure = Figure(figsize=(6, 4), dpi=100)

        # Pressure subplot
        self.press_ax: plt.Axes = self.env_fig.add_subplot(211)  # type: ignore
        self.press_line: plt.Line2D
        self.press_line, = self.press_ax.plot([], [], 'b-')
        self.press_ax.set_ylabel('Pressure (hPa)')
        self.press_ax.grid(True)

        # Humidity subplot
        self.hum_ax: plt.Axes = self.env_fig.add_subplot(212)  # type: ignore
        self.hum_line: plt.Line2D
        ...
        self.env_canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.env_fig, master=parent)
        self.env_canvas.get_tk_widget().pack(fill='both', expand=True)

    def setup_battery_plot(self, parent: tk.Frame) -> None:
        """Setup battery plot"""
        self.bat_fig: Figure = Figure(figsize=(6, 4), dpi=100)
        self.bat_ax: plt.Axes = self.bat_fig.add_subplot(111)  # type: ignore
        self.bat_line: plt.Line2D
        ...
        self.bat_canvas: FigureCanvasTkAgg = FigureCanvasTkAgg(self.bat_fig, master=parent)
    ...
    def update_plots(self) -> None:
        ...
        t0: float = self.time_data[0]
        t_norm: List[float] = [t - t0 for t in self.time_data]
