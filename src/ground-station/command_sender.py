...
class CommandSender:
    ...
    def setup_gui(self) -> None:
        ...
        self.command_combo: ttk.Combobox = ttk.Combobox(input_frame, textvariable=self.command_var,
        ...
        for i, (label, cmd) in enumerate(quick_buttons):
            btn: ttk.Button = ttk.Button(quick_frame, text=label,
        ...
        self.history_text: scrolledtext.ScrolledText = scrolledtext.ScrolledText(history_frame, height=15)  # type: ignore
        self.history_text.pack(fill='both', expand=True, padx=5, pady=5)

        # Bind double-click to re-send
        self.history_text.bind('<Double-Button-1>', self.resend_command)

    def on_command_selected(self, event: tk.Event) -> None:  # type: ignore
    ...
    def resend_command(self, event: tk.Event) -> None:  # type: ignore
