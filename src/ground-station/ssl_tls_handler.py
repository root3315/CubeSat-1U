...
class GroundStationSSLHandler:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        for cert_path in [self.cert_file, self.key_file, self.ca_cert_file]:
            certs_dir: str = os.path.dirname(cert_path)
    ...
    def wrap_socket(
        ...
        try:
            wrapped_sock: ssl.SSLSocket = self.ssl_context.wrap_socket(
                sock,
    ...
    def create_secure_server_socket(self, host: str, port: int) -> ssl.SSLSocket:
        """
        ...
        # Wrap with SSL
        ssl_sock: ssl.SSLSocket = self.wrap_socket(server_sock, server_side=True)
        ssl_sock.bind((host, port))

        return ssl_sock

    def create_secure_client_socket(self, host: str, port: int) -> ssl.SSLSocket:
        """
        ...
        # Wrap with SSL
        ssl_sock: ssl.SSLSocket = self.wrap_socket(client_sock, server_side=False)
